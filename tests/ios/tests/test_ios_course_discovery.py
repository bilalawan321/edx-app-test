...
from tests.ios.pages.ios_course_discovery import IosCourseDiscovery
...
class TestIosLogin:
    ...
    def test_start_login_smoke(self, login, set_capabilities, setup_logging):
        """
        Scenario:
    ...
    def test_ui_elements_smoke(self, set_capabilities, setup_logging):
        """
        Scenarios:
                Verify that Discovery tab will show following Header contents,
                Profile icon,
                Discovery edit-field in place of Main Dashboard title,
                Search icon,
                Account Icon
        """

        ios_course_discovery_page = IosCourseDiscovery(set_capabilities, setup_logging)
        ios_course_discovery_page.get_discovery_tab().click()
        assert ios_course_discovery_page.get_profile_icon().text == strings.PROFILE_SCREEN_TITLE
        assert ios_course_discovery_page.get_search_icon().text == strings.COURSES_DISCOVERY_SEARCH_ICON_IOS
        assert ios_course_discovery_page.get_browse_by_subject_icon().text == strings.COURSES_DISCOVERY_BROWSE_BY_SUBJECT_LABEL
        ios_course_discovery_page.get_filter_courses_view(set_capabilities)

        # ios_main_dashboard_page = IosMainDashboard(set_capabilities, setup_logging)
        # ios_main_dashboard_page.get_discovery_tab().click()
        # assert ios_main_dashboard_page.get_profile_icon().text == strings.PROFILE_SCREEN_TITLE
        # assert ios_main_dashboard_page.get_search_icon().text == strings.COURSES_DISCOVERY_SEARCH_ICON_IOS
        # assert ios_main_dashboard_page.get_browse_by_subject_icon().text == strings.COURSES_DISCOVERY_BROWSE_BY_SUBJECT_LABEL
        # ios_main_dashboard_page.get_available_courses_number()
        # set_capabilities.back()
        # print('222222:', ios_main_dashboard_page.get_account_options().text)
        # assert ios_main_dashboard_page.get_account_options().text == strings.ACCOUNT_SCREEN_TITLE
        # set_capabilities.back()
